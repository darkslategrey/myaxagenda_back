 ...
 */
Ext.define('Ext.ux.calendar.view.MonthView', {
	...
	initMenu : function() {
		...
		this.copyItem = Ext.create('Ext.menu.Item',{
		...
		this.pasteItem = Ext.create('Ext.menu.Item',{
					iconCls : 'icon_feyaCalendar_paste',
					text : lan['pasteItem.text'],
					handler : this.onPasteFn,
					scope : this
				});
		this.menu = new Ext.menu.Menu({items : [this.addItem, this.clearItem, '-', this.cutItem,this.copyItem, this.pasteItem]});
	...
	generateHTML : function(data) {
		...
		var header = [];
		header.push('<div id="', this.id,'-x-monthview-header" class="x-monthview-header" unselectable="on">');
	...
	initComponent : function() {
		...
		this.addEvents('checklayout', 'afterresize', 'beforeremoteload','remoteload', 'canceldetail', 'viewDay', 'viewWeek');
