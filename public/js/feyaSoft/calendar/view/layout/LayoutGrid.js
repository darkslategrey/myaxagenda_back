...
Ext.ux.calendar.LayoutGrid.prototype = {    
    ...
    getEventList: function(config){
	if(config != undefined) {
	    console.log("LayoutGrid.getAllEvents: uaValue <"+config.uaValue+"> utd.Value <"+config.utdValue+">");
	}
    ...
    reLayout: function(config, single, nowhole){
	return this.generateLayout(this.getEventList(config), true, single, nowhole);
    },
	...
	console.log("LayoutGrid.generateLayout reload <"+reload+">");
        var elist = [], wlist = [];
        if(!reload){
            this.heventList = [];            
            elist = eventList;
